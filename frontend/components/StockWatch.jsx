...
import {matchingWatchState,thesisItems,hasThesisBlock,thesisAxes,thesisPhase,canResumeDraft} from '../lib/stock-watch.mjs';
...
export default function StockWatch({data,onBusyChange,externalBusy=false}){
 ...
 const [state,setState]=useState(null),[checking,setChecking]=useState(true),[driverError,setDriverError]=useState(''),[working,setWorking]=useState('');
 const [seconds,setSeconds]=useState(0),[serverBusy,setServerBusy]=useState(false);
 ...
 async function load(signal){
  ...
  try{
   ...
   if(alive.current){setState(matched);setServerBusy(Boolean(status.busy));setDriverError('');}
 ...
 async function run(kind){
  ...
  if(kind==='drivers'&&!window.confirm('투자 논리를 자동 조사합니다. 웹 조사 1회(웹 도구 최대 8회)와 정리 1회로 총 최대 2회 API 비용이 발생합니다. 티커와 공개자료를 OpenAI에 전송합니다. 자동 유료 재시도는 없습니다. 진행할까요?'))return;
  if(kind==='resume'&&!window.confirm('저장 조사 초안만 다시 정리합니다. 새 웹 검색 없이 최대 1회 API 비용이 발생합니다. 진행할까요?'))return;
  flight.current=true;setWorking(kind);setDriverError('');
  const ac=new AbortController(),timer=setTimeout(()=>ac.abort(),395000);
  try{
   ...
   if(!status.configured)throw new Error('API 키 설정이 필요합니다. Configure-AI.ps1을 실행하세요.');
   const body={snapshot_id:data.research_snapshot_id,allow_paid:true,refresh:true,max_paid_calls:kind==='resume'?1:2,reuse_research:kind==='resume'};
   const result=await json('/api/v1/watch/drivers',{method:'POST',headers:{'Content-Type':'application/json','X-SignalDesk-AI':status.csrf_token},body:JSON.stringify(body),signal:ac.signal});
  ...
  }catch(e){if(alive.current){setDriverError(e.name==='AbortError'?'응답 대기가 끝났습니다. 서버 요청은 진행 중일 수 있습니다. 기존 결과를 유지하며 자동 재시도하지 않습니다.':e.message);setState(old=>({...old,last_thesis_attempt:{outcome:'FAILED',error_code:e.code}}));}}
 ...
 const report=state?.drivers,items=thesisItems(report,symbol),draft=state?.research_draft;
 ...
 return <section className="ds-section sw-section" id="desk-drivers" aria-label="기업별 투자 논리">
  <div className="ds-section-head"><div><span className="eyebrow">03 / INVESTMENT THESIS</span><h2>{symbol} — 이 회사의 투자 논리는?</h2><p>사업 구조와 핵심 변수를 조사해, 무엇이 확인되면 투자 논리가 강해지거나 틀어지는지 정리합니다.</p></div><button type="button" className="ds-text-button" disabled={disabled} onClick={()=>void load()}>저장본 읽기</button></div>
  ...
  {report?.financing&&<div id="sd-thesis-funding" className="th-funding"><h3>자금조달에서 봐야 할 것</h3><p>{report.financing.current_state}</p><p><b>위험이 커지는 조건</b> {report.financing.risk_condition??'조건을 충분히 연결하지 못했습니다.'}</p><small>공개자료 기반 AI 해석입니다. 실제 발행 완료 여부나 희석률을 확정한 결과가 아닙니다.</small><Evidence value={report.financing}/></div>}
