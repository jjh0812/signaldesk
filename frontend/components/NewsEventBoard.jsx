...
export function SignalEventCard({entry,report}) {
  const item=eventInsight(entry,report);
...
export default function NewsEventBoard({events=[],report=null,cards=[],loading=false,horizon='all'}) {
  const news=splitNewsCards(cards);
  return <div data-testid="news-event-board" data-ui-feature="direction-conditions-v106">
    <div className="ds106-news-grid"><NewsColumn type="positive" title="긍정 뉴스·근거" cards={news.positive} loading={loading}/><NewsColumn type="negative" title="부정 뉴스·리스크" cards={news.negative} loading={loading}/></div>
    <section className="ds106-upcoming"><header className="ds106-upcoming-head"><div><h3><i aria-hidden="true"/>{horizon==='unknown'?'날짜 미확인 이벤트':'다가오는 이벤트·확인할 항목'}</h3><p>먼저 현재 판단을 읽고, 아래 두 줄에서 호재·악재가 되는 조건을 확인하세요.</p></div><b>{events.length}</b></header>
      {events.length?<>{events.slice(0,3).map(e=><SignalEventCard key={e.id} entry={e} report={report}/>)}{events.length>3&&<details className="ds106-fact ds106-more"><summary>나머지 이벤트 {events.length-3}개</summary>{events.slice(3).map(e=><SignalEventCard key={e.id} entry={e} report={report}/>)}</details>}</>:<p className="ds106-news-empty">{loading?'저장 이벤트 확인 중…':'선택한 범위에 표시할 근거를 확보하지 못했습니다. 확정된 일정이 없다는 의미는 아닙니다.'}</p>}
    </section>
